(function ($) {
	function showMemberLevelFields() {
		...
		$toggle.on('click', function (e) {
			...
			if (checkbox.is(':checked')) {
				...
				$(
					'.minnpost-member-field.minnpost_membership_frequency_options, .minnpost-member-field.minnpost_membership_default_frequency'
				).show();
			} else {
	...
	function toggleMembershipChangeFields() {
		const $toggle = $(
			'#minnpost_membership_support_post_form_change_for_members'
		);
		const fields =
			'.minnpost_membership_support_post_form_nochange, .minnpost_membership_support_post_form_change';
	...
	$(document).ready(function () {
		...
		$('.minnpost-membership-general-settings').minnpostMembership({
			...
			frequency_selector_in_levels:
				'input[name="minnpost_membership_default_frequency[]"]',
