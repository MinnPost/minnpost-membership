...
const config = {
  ...
  scripts: {
    ...
    util: "./assets/js/src/util/minnpost-membership.js",
    util_lint: "./assets/js/src/util/",
...
function frontendscripts() {
	return gulp
		.src([config.scripts.util, config.scripts.front_end])
		.pipe(sourcemaps.init())
		.pipe(
			babel({
				presets: ["@babel/preset-env"]
...
function utilscriptlint() {
	return gulp
		.src(config.scripts.util)
		.pipe(eslint( {
				fix:true,
				parserOptions: {
					requireConfigFile: false
		...
		.pipe(eslint.format())
		.pipe(gulp.dest(config.scripts.util_lint))
		// Brick on failure to be super strict
		//.pipe(eslint.failOnError());
};
...
const lint = gulp.series(adminsasslint, frontendsasslint, adminscriptlint, utilscriptlint, frontendscriptlint);
